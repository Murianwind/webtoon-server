...
**`volumes` 한 줄의 의미**: `"호스트의_실제_경로:/library/원하는_이름"` 형식입니다.
콜론(`:`) **왼쪽**은 내 컴퓨터에 실제로 있는 웹툰 폴더 경로, **오른쪽**(`/library/` 뒤)은
이 앱이 그 폴더를 부르는 이름이고 화면에 그대로 태그로 표시됩니다. 오른쪽 이름은
`naver`/`kakao`로 고정된 게 아니라 완전히 자유롭게 지어도 됩니다.
...
이렇게 적으면 화면에 "카카오"라는 태그로 표시됩니다. `/library/kakao`라고 적으면
태그도 `kakao`로 그대로 표시됩니다 — 오른쪽 이름을 뭘로 짓든 화면에 그 글자 그대로
나오는 것뿐이라, 원하시는 이름으로 자유롭게 쓰시면 됩니다.
...
`volumes`에 이런 줄을 몇 개 두느냐가 곧 플랫폼(웹툰 사이트) 폴더 개수입니다.
