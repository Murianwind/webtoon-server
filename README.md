...
- 라이브러리 자동/수동 재스캔
- 웹 UI에서 라이브러리(플랫폼 폴더) 등록/등록 해제 — 해제해도 실제 폴더/zip 파일은 삭제하지 않음
...
- **등록된 라이브러리**: `/library` 아래 실제로 스캔 중인 폴더 목록. "삭제"를 눌러도
  등록 목록에서만 빠질 뿐, 실제 폴더나 zip 파일은 절대 지워지지 않습니다.
- **등록 가능한 폴더**: `/library` 아래엔 있지만 아직 스캔 대상이 아닌 폴더. "등록"을
  누르면 바로 시리즈 목록에 반영됩니다.
- **백업/복원**: 읽음 진행률, 검색/정렬/필터 설정, 라이브러리 등록 상태를 JSON 파일
